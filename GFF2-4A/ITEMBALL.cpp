#include "ITEMBALL.h"

ITEMBALL::ITEMBALL() 
{
	map_x = 0;
	map_y = 0;
	player = nullptr;
	item_x = 0;
	item_y = 0;
	item = 0;
}

ITEMBALL::ITEMBALL(float enemy_x, float enemy_y,int e_mapy,int e_mapx,PLAYER* r_player,STAGE* r_stage)
{
	if (item = (LoadGraph("Resource/Images/Enemy/ThrowSlime.png")) == -1)
	{
		throw "Resource/Images/Enemy/ThrowSlime.png";
	}
	map_x = e_mapx;
	map_y = e_mapy;
	player = r_player;
	stage = r_stage;
	item_x = enemy_x;
	item_y = enemy_y;
}

ITEMBALL::~ITEMBALL() {};

void ITEMBALL::Draw() const 
{
	DrawRotaGraph(item_x, item_y, 1, 0, item, TRUE);
}

void ITEMBALL::Update()
{
	if ((item_x + stage->GetScrollX() > 0) && (item_x + stage->GetScrollX() < 1280))
	{
		PlayerHit();
	}
}

void ITEMBALL::PlayerHit()
{
	float px1, py1, px2, py2;
	float bx1, by1, bx2, by2;

	px1 = player->GetPlayerX() - 30;
	px2 = px1 + 60;
	py1 = player->GetPlayerY();
	py2 = py1 + 40;

	bx1 = item_x+_BALL_RADIUS;
	bx2 = bx1 + _BALL_RADIUS;
	by1 = item_y + _BALL_RADIUS;
	by2 = by1 + _BALL_RADIUS;

	if (px1 < bx2 && bx1 < px2 && py1 < by2 && by1 < py2)
	{
		delete_flg = true;
	}
	if (stage->HitMapDat(map_y, map_x))
	{
		delete_flg = true;
		stage->HitMapDat(map_y, map_x);
	}
}
